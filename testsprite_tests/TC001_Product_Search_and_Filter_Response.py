...
async def run_test():
    ...
    try:
        ...
        # -> Input valid login credentials and submit to access main catalog page.
        ...
        frame = context.pages[-1]
        try:
            await expect(frame.locator('text=Search results match criteria within 1 second').first).to_be_visible(timeout=1000)
        except AssertionError:
            raise AssertionError('Test case failed: The product search and filtering did not return accurate results within 1 second as required by the test plan.')
